...
export const prepareDataForWebhook = async (betObj: BetsData, key: WebhookKey): Promise<WebhookData | false> => {
    try {
        ...
        if (key == 'DEBIT') return {
            ...
            description: `${Number(bet_amount).toFixed(2)} debited for color game for Round ${id}`,
            bet_id,
            txn_type: 0
        }
            ...
            txn_ref_id: txn_id,
            description: `${Number(winning_amount).toFixed(2)} credited for color game for Round ${id}`,
            txn_type: 1
        }
